...
test('settings screen is a first-class academy-style route and title controls add 設定 while shrinking/moving the title shell to the upper-left', async () => {
  ...
  assert.match(css, /\.settings-screen-shell[\s\S]*height:\s*100%[\s\S]*grid-template-rows:\s*auto minmax\(0, 1fr\)/, 'settings screen shell should follow the academy viewport-fit layout family');
  assert.match(css, /\.settings-card[\s\S]*border:\s*1px solid rgba\(211, 180, 105, 0\.34\)[\s\S]*linear-gradient\(180deg, rgba\(8, 12, 20, 0\.94\), rgba\(4, 7, 12, 0\.98\)\)/, 'settings card should align with the academy room / conversation-session dark-gold card palette');
