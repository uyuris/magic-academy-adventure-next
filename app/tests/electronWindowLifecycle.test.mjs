...
test('resolveMainWindowEntryUrl keeps runtimeUrl raw for non-packaged runs and adds title override only for packaged entry', async () => {
  assert.equal(resolveMainWindowEntryUrl({ runtimeUrl: 'http://127.0.0.1:41731', isPackaged: false }), 'http://127.0.0.1:41731');
  assert.equal(resolveMainWindowEntryUrl({ runtimeUrl: 'http://127.0.0.1:41731', isPackaged: true }), 'http://127.0.0.1:41731/?initialScreen=title');
});
