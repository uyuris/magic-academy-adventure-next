...
import { cloneGameDataFixture } from './helpers.mjs';
import { definitionsRoot, projectRoot } from './testPaths.mjs';
import { finalizeConversation, runConversationOpening } from '../src/llm/conversationPipeline.mjs';
...
async function withServer(t, root) {
  const server = createServer({ root, publicRoot: livePublicRoot });
...
test('conversation finalization turns an agreed next destination into a dynamic event flag with location_id output prompt', async () => {
  const root = await fixtureRoot();
  ...
  const result = await finalizeConversation({
    ...
    characterId: 'lina',
    now: '2026-05-06T13:11:00.000+09:00',
    memoryUpdateProvider: async ({ conversation, workRecordId }) => ({ memory_record: { id: 'mem_destination_test', character_id: conversation.character_id, type: 'relationship_change', text: '主人公とリナは次に図書館へ向かう段取りを決めた。', source_conversation_id: conversation.id, work_record_id: workRecordId, visibility: 'character_known', tags: [] } }),
    skillUpdateProvider: async ({ conversation, workRecordId }) => ({ skill_record: { id: 'skill_destination_test', character_id: conversation.character_id, type: 'self_change', name: '次の移動先を決めた', description: 'リナは主人公と次の移動先を具体的に決めた。', source_conversation_id: conversation.id, work_record_id: workRecordId, visibility: 'character_known', tags: [] } }),
    workRecordProvider: async ({ conversation, workRecordId }) => ({ work_record: { id: workRecordId, character_id: conversation.character_id, source_conversation_id: conversation.id, title: '次の移動先を決めた', summary: '主人公とリナはこの会話のあとに大図書館の閲覧ホールへ行くことを合意した。', flag_update_candidates: [], warnings: [] } }),
    stageFlagJudgmentProvider: async () => ({ flag_results: [] }),
    eventFlagJudgmentProvider: async () => ({ flag_results: [] }),
    eventParticipantOverrideJudgmentProvider: async () => ({ flag_results: [] }),
    ...
    destinationStageProvider: async ({ prompt }) => {
      destinationPrompts.push(prompt);
      return 'library_reading_room';
    },
    skillFlowRollProvider: () => 0.1
  });

  assert.equal(destinationPrompts.length, 1);
  assert.match(destinationPrompts[0], /返答は対応表にあるlocation_idを1つだけ返す/);
  assert.match(destinationPrompts[0], /大図書館の閲覧ホール: library_reading_room/);
  assert.doesNotMatch(destinationPrompts[0], /禁書庫前: forbidden_archive_door/);

  assert.equal(result.destination_stage_update.location_id, 'library_reading_room');
  assert.equal(result.destination_stage_update.flag_id, 'event.destination_stage.conv_forbidden_archive_teatime.ready');
  assert.equal(result.destination_stage_update.completed_flag_id, 'event.destination_stage.conv_forbidden_archive_teatime.completed');

  const state = await readJson(root, 'game_data/runtime_state.json');
  assert.equal(state.global_flags['event.destination_stage.conv_forbidden_archive_teatime.ready'], true);
  assert.deepEqual(state.event_flag_sources['event.destination_stage.conv_forbidden_archive_teatime.ready'], {
    character_id: 'lina',
    conversation_id: 'conv_forbidden_archive_teatime',
    achieved_at: '2026-05-06T13:11:00.000+09:00'
  });
  assert.deepEqual(state.pending_destination_stage_event, {
    flag_id: 'event.destination_stage.conv_forbidden_archive_teatime.ready',
    completed_flag_id: 'event.destination_stage.conv_forbidden_archive_teatime.completed',
    character_id: 'lina',
    character_name: 'リナ・クラウゼ',
    conversation_id: 'conv_forbidden_archive_teatime',
    location_id: 'library_reading_room',
    location_name: '大図書館の閲覧ホール',
...
test('event flag status merges dynamic destination-stage events and all-off clears them', async () => {
  const root = await fixtureRoot();
  const state = await readJson(root, 'game_data/runtime_state.json');
  state.global_flags['event.destination_stage.conv_forbidden_archive_teatime.ready'] = true;
  state.event_flag_sources = {
    'event.destination_stage.conv_forbidden_archive_teatime.ready': {
      character_id: 'lina',
      conversation_id: 'conv_forbidden_archive_teatime',
      achieved_at: '2026-05-06T13:11:00.000+09:00'
    }
  };
  state.pending_destination_stage_event = {
    flag_id: 'event.destination_stage.conv_forbidden_archive_teatime.ready',
    completed_flag_id: 'event.destination_stage.conv_forbidden_archive_teatime.completed',
    character_id: 'lina',
    character_name: 'リナ・クラウゼ',
  ...
  const dynamicEvent = status.pending_events.find((event) => event.id === 'event.destination_stage.conv_forbidden_archive_teatime.ready');
  assert.equal(dynamicEvent.label, '会話後移動: 大図書館の閲覧ホール');
  assert.equal(dynamicEvent.type, 'destination_stage');
  assert.equal(dynamicEvent.auto_generated, true);
  assert.equal(dynamicEvent.interaction.location_id, 'library_reading_room');
  assert.equal(dynamicEvent.character_id, 'lina');

  await setAllEventFlagsActive({ root, active: false });
  const clearedState = await readJson(root, 'game_data/runtime_state.json');
  assert.equal(clearedState.pending_destination_stage_event ?? null, null);
  assert.equal(clearedState.global_flags['event.destination_stage.conv_forbidden_archive_teatime.ready'] ?? false, false);
  assert.equal(clearedState.global_flags['event.destination_stage.conv_forbidden_archive_teatime.completed'] ?? false, false);
  assert.equal(clearedState.event_flag_sources?.['event.destination_stage.conv_forbidden_archive_teatime.ready'] ?? null, null);

  const clearedStatus = await getEventFlagStatus({ root });
  assert.equal(clearedStatus.pending_events.some((event) => event.id === 'event.destination_stage.conv_forbidden_archive_teatime.ready'), false);
  await fs.rm(root, { recursive: true, force: true });
});

test('starting a dynamic destination-stage event moves to the agreed location and seeds interaction context', async () => {
  const root = await fixtureRoot();
  const state = await readJson(root, 'game_data/runtime_state.json');
  state.global_flags['event.destination_stage.conv_forbidden_archive_teatime.ready'] = true;
  state.event_flag_sources = {
  ...
  const started = await startEventFlagInteraction({ root, flagId: 'event.destination_stage.conv_forbidden_archive_teatime.ready', screen: 'academy-conversation-session' });
  assert.equal(started.character_id, 'lina');
  assert.equal(started.location_id, 'library_reading_room');
  assert.equal(started.state.current_screen, 'academy-conversation-session');
  assert.equal(started.state.current_location_id, 'library_reading_room');
  assert.equal(started.state.current_interaction_character_id, 'lina');
  assert.equal(started.state.pending_interaction_context.source_type, 'destination_stage');
  assert.equal(started.state.pending_interaction_context.event_flag_id, 'event.destination_stage.conv_forbidden_archive_teatime.ready');
  assert.equal(started.state.pending_interaction_context.source_conversation_id, 'conv_forbidden_archive_teatime');
  assert.equal(started.state.pending_interaction_context.opening_context, '二人は会話の中で合意した場所へ移動してきた。');
  assert.equal(started.state.global_flags['event.destination_stage.conv_forbidden_archive_teatime.completed'], true);
  assert.equal(started.state.pending_destination_stage_event ?? null, null);
  assert.match(started.state.current_location_visible_situation, /大図書館の閲覧ホール/);
...
test('meal, leyline, snow, rain, and mirror events become ready and pending when judged as achieved', async () => {
  const cases = [
    {
      readyFlagId: 'event.shared_meal_promise.ready',
      completedFlagId: 'event.shared_meal_promise.completed',
      locationId: 'student_cafeteria_magic_lamps',
      prompt: '学生食堂で会話する。',
      conversationId: 'conv_shared_meal_source',
      label: '一緒に食事する約束',
      memoryText: 'リナは主人公と一緒に食事をする約束をした。',
      workRecordTitle: '一緒に食事する約束をした',
      workRecordSummary: '主人公とリナは一緒に食事をする約束をした。'
    },
    {
      readyFlagId: 'event.leyline_inspection_promise.ready',
      completedFlagId: 'event.leyline_inspection_promise.completed',
      locationId: 'underground_waterway',
      prompt: '地下水路で会話する。',
      conversationId: 'conv_leyline_inspection_source',
      label: '地脈確認の約束',
      memoryText: 'リナは主人公と地脈の様子を確認する約束をした。',
      workRecordTitle: '地脈確認の約束をした',
      workRecordSummary: '主人公とリナは地脈の様子を確認する約束をした。'
    },
    {
...
test('monster subjugation promise event completes when its event conversation ends without an LLM completion judgment', async () => {
  const root = await fixtureRoot('magic-adv-monster-subjugation-complete-on-end-');
  const state = await readJson(root, 'game_data/runtime_state.json');
  state.current_location_id = 'crystal_cave_below_school';
  state.current_interaction_character_id = 'lina';
  state.global_flags['event.monster_subjugation_promise.ready'] = true;
  state.global_flags['event.monster_subjugation_promise.completed'] = false;
  state.event_flag_sources = {
    'event.monster_subjugation_promise.ready': {
      character_id: 'lina',
      conversation_id: 'conv_monster_subjugation_promise_source',
      achieved_at: '2026-05-08T14:00:00.000+09:00'
    }
  };
  state.pending_interaction_context = {
    source_type: 'event',
    event_flag_id: 'event.monster_subjugation_promise.ready',
    event_label: '魔物討伐の約束',
    source_conversation_id: 'conv_monster_subjugation_promise_source'
  };
  await fs.writeFile(path.join(root, 'game_data/runtime_state.json'), JSON.stringify(state, null, 2), 'utf8');
  await fs.writeFile(path.join(root, 'game_data/logs/conversations/conv_monster_subjugation_event.json'), JSON.stringify({
    id: 'conv_monster_subjugation_event',
    character_id: 'lina',
    character_name: 'リナ・クラウゼ',
    created_at: '2026-05-08T14:10:00.000+09:00',
    updated_at: '2026-05-08T14:20:00.000+09:00',
    source_type: 'event',
    location_id: 'crystal_cave_below_school',
    time_slot: 'after_school',
    prompt: '学院地下の水晶洞で会話する。',
    messages: [
      { role: 'user', content: '浅い層から探って、危ない反応が出たらすぐに下がろう。' },
      { role: 'assistant', content: 'リナは結晶の光を見回しながら、まずは足跡と魔力痕を一緒に確かめようと応じた。' }
    ]
  }, null, 2), 'utf8');
  ...
  const result = await finalizeConversation({
    root,
    conversationId: 'conv_monster_subjugation_event',
    characterId: 'lina',
    now: '2026-05-08T14:21:00.000+09:00',
    memoryUpdateProvider: async ({ conversation, workRecordId }) => ({ memory_record: { id: 'mem_monster_subjugation_end', character_id: conversation.character_id, type: 'relationship_change', text: 'リナは主人公と魔物討伐に向かった。', source_conversation_id: conversation.id, work_record_id: workRecordId, visibility: 'character_known', tags: [] } }),
    skillUpdateProvider: async () => ({ skipped: true }),
    workRecordProvider: async ({ conversation, workRecordId }) => ({ work_record: { id: workRecordId, character_id: conversation.character_id, source_conversation_id: conversation.id, title: '魔物討伐の約束イベントを終えた', summary: '主人公とリナは魔物討伐の約束イベントを終えた。', flag_update_candidates: [], warnings: [] } }),
    stageFlagJudgmentProvider: async () => ({ flag_results: [] }),
    eventFlagJudgmentProvider: async () => ({ flag_results: [] }),
    eventCompletionJudgmentProvider: async () => { completionProviderCalled = true; return { flag_results: [] }; },
    skillFlowRollProvider: () => 1
  ...
  assert.equal(completionProviderCalled, false, 'conversation-end completion should not ask the LLM to rejudge the event');
  assert.equal(result.event_completions.accepted[0].completed_flag_id, 'event.monster_subjugation_promise.completed');
  const nextState = await readJson(root, 'game_data/runtime_state.json');
  assert.equal(nextState.global_flags['event.monster_subjugation_promise.completed'], true);
  assert.deepEqual(nextState.event_completion_sources['event.monster_subjugation_promise.completed'], {
    event_flag_id: 'event.monster_subjugation_promise.ready',
    character_id: 'lina',
    conversation_id: 'conv_monster_subjugation_event',
    achieved_at: '2026-05-08T14:21:00.000+09:00',
    completed_on_conversation_end: true
  });
  const completionLog = await readJson(root, 'game_data/logs/event_completion_judgments/conv_monster_subjugation_event.json');
  assert.deepEqual(completionLog.accepted.map((flag) => flag.completed_flag_id), ['event.monster_subjugation_promise.completed']);
  await fs.rm(root, { recursive: true, force: true });
});
...
test('meal, leyline, snow, rain, and mirror events complete on conversation end without an LLM completion judgment', async () => {
  const cases = [
    {
      rootPrefix: 'magic-adv-shared-meal-complete-on-end-',
      readyFlagId: 'event.shared_meal_promise.ready',
      completedFlagId: 'event.shared_meal_promise.completed',
      locationId: 'student_cafeteria_magic_lamps',
      eventLabel: '一緒に食事する約束',
      sourceConversationId: 'conv_shared_meal_promise_source',
      conversationId: 'conv_shared_meal_event',
      achievedAt: '2026-05-08T15:30:00.000+09:00',
      createdAt: '2026-05-08T15:40:00.000+09:00',
      updatedAt: '2026-05-08T15:50:00.000+09:00',
      now: '2026-05-08T15:51:00.000+09:00',
      timeSlot: 'after_school',
      prompt: '学生食堂で会話する。',
      userMessage: '今日のおすすめを半分ずつ試しながら、ゆっくり食べよう。',
      assistantMessage: 'リナは盆を運び、味の感想も聞かせてほしいと微笑んだ。',
      memoryText: 'リナは主人公と一緒に食事をした。',
      workRecordTitle: '一緒に食事する約束イベントを終えた',
      workRecordSummary: '主人公とリナは一緒に食事する約束イベントを終えた。'
    },
    {
      rootPrefix: 'magic-adv-leyline-inspection-complete-on-end-',
      readyFlagId: 'event.leyline_inspection_promise.ready',
      completedFlagId: 'event.leyline_inspection_promise.completed',
      locationId: 'underground_waterway',
      eventLabel: '地脈確認の約束',
      sourceConversationId: 'conv_leyline_inspection_promise_source',
      conversationId: 'conv_leyline_inspection_event',
      achievedAt: '2026-05-08T16:00:00.000+09:00',
      createdAt: '2026-05-08T16:10:00.000+09:00',
      updatedAt: '2026-05-08T16:20:00.000+09:00',
      now: '2026-05-08T16:21:00.000+09:00',
      timeSlot: 'after_school',
      prompt: '地下水路で会話する。',
      userMessage: '流れの脈動が乱れていないか、分岐ごとに順番に見ていこう。',
      assistantMessage: 'リナは水面に灯りを落とし、反応の揺れを一緒に見比べようと応じた。',
      memoryText: 'リナは主人公と地脈の様子を確認した。',
      workRecordTitle: '地脈確認の約束イベントを終えた',
      workRecordSummary: '主人公とリナは地脈確認の約束イベントを終えた。'
    },
    {
...
test('monster subjugation promise event definition has no judgment prerequisites and uses the agreed crystal cave route', async () => {
  const root = await fixtureRoot('magic-adv-monster-subjugation-definition-');
  const definitions = await loadEventFlags({ root });
  const flag = definitions.flags.find((entry) => entry.id === 'event.monster_subjugation_promise.ready');

  assert.equal(flag.label, '魔物討伐の約束');
  assert.equal(flag.condition, '一緒に魔物討伐に行くことに合意すること。');
  assert.equal(flag.question, '主人公と会話相手が一緒に魔物討伐に行くことに合意したか');
  assert.deepEqual(flag.required_global_flags, []);
  assert.deepEqual(flag.required_inventory_items, []);
  assert.equal(flag.completed_flag_id, 'event.monster_subjugation_promise.completed');
  assert.equal(flag.complete_on_conversation_end, true);
  assert.equal(flag.interaction.location_id, 'crystal_cave_below_school');
  assert.equal(flag.interaction.source_type, 'event');
  assert.equal(selectEventFlagJudgmentTargets({
    flags: definitions.flags,
    state: { global_flags: {}, event_flag_sources: {} },
    inventory: { items: [] },
    conversation: { character_id: 'lina' }
  }).map((entry) => entry.id).includes('event.monster_subjugation_promise.ready'), true);
  await fs.rm(root, { recursive: true, force: true });
});
...
test('meal, leyline, snow, rain, and mirror event definitions use the agreed routes and remain conversation-end judgments', async () => {
...
test('configured event interactions preserve event source type for every playable event conversation opening', async () => {
  ...
  const playableEvents = [
    ...
    },
    {
      ...
      label: '魔物討伐の約束',
      completedFlagId: 'event.monster_subjugation_promise.completed',
      locationId: 'crystal_cave_below_school',
      openingContext: '魔物討伐の約束イベント。'
    },
    {
