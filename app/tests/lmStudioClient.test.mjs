...
test('createLmStudioProviders returns chat and separate continuity update providers for the conversation pipeline', async (t) => {
  ...
  const memoryPromptLines = requests[4].body.messages[0].content.trim().split('\n');
  const skillNecessityPromptLines = requests[5].body.messages[0].content.trim().split('\n');
  const skillPromptLines = requests[6].body.messages[0].content.trim().split('\n');
  const workRecordPromptLines = requests[7].body.messages[0].content.trim().split('\n');
  ...
  for (const prompt of [requests[4].body.messages[0].content, requests[5].body.messages[0].content, requests[6].body.messages[0].content, requests[7].body.messages[0].content]) {
  ...
  assert.doesNotMatch(requests[7].body.messages[0].content, /future_hooks/);
  assert.equal(requests[7].body.response_format, undefined);
});
