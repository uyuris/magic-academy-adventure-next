...
function normalizePaths({ root, ...paths } = {}) {
  ...
  if (useLegacyProjectLayout) {
    ...
    if (inheritedRuntime) {
      ...
      return {
        ...
        resourceRoot: path.resolve(paths.resourceRoot ?? manifest.resourceRoot)
      };
    }
    return {
      projectRoot: resolvedProjectRoot,
  ...
  return {
    ...
    resourceRoot: path.resolve(paths.resourceRoot ?? resolvedProjectRoot)
  };
}
...
async function resolveLegacyReadPath(paths, relativePath) {
  ...
  if (rest === 'runtime_state.json' || rest === 'player_inventory.json' || rest === path.join('runtime', 'player_parameters.json')) {
  ...
  if (rest.startsWith(`characters${path.sep}`)) {
    ...
    if (leaf === 'skills.json' || leaf === 'flags.json' || leaf === 'memory' || leaf.startsWith(`memory${path.sep}`) || leaf === 'work_records' || leaf.startsWith(`work_records${path.sep}`)) {
...
export function createStorageApi(options = {}) {
  ...
  async function readCharacter(characterId) {
    ...
    return {
      ...
      skills: skills ?? { character_id: characterId, skills: [] }
    };
  }
  ...
  async function listMarkdownRecords(relativeDir) {
    ...
    return Promise.all(mdNames.map(async (name) => ({
      ...
      body: await fs.readFile(path.join(fullDir, name), 'utf8'),
      tags: []
    })));
  }
