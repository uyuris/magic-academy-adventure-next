...
export function createLmStudioProviders({ config, fetchImpl, onChatDelta } = {}) {
  ...
  return {
    ...
    conversationContinuationProvider: async ({ prompt }) => parseBooleanOnlyAnswer(await callLmStudioReflectionText({
      config,
      prompt,
      fetchImpl,
      title: '会話終了判定'
    ...
    destinationStageProvider: ({ prompt }) => callLmStudioReflectionText({
      config,
      prompt,
      fetchImpl,
      title: '会話後移動先合意判定'
