function flagExists({ state, characterId, flag }) {
  ...
  const characterFlags = state.characters?.[characterId]?.flags ?? {};
  return Object.prototype.hasOwnProperty.call(characterFlags, flag);
}
...
function validateMemoryRecord(candidate, state) {
  if (!candidate?.character_id || !state.characters?.[candidate.character_id]) return 'unknown character';
